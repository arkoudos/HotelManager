...
public class Trie {
    ...
    int counter = 1;
    static private int printSearch = 1;

    public static void setPrintSearch(int printSearch) {
        Trie.printSearch = printSearch;
    }
            ...
            return false; //the word isn't in the tree
                ...
                if (p.isEnd) {
                    ...
                    for (int i = 0; i < p.index; i++) {
                        if (printSearch == 1){
                        CreateMenu.output.append("Number of compares: " + counter + i + ", The Hotel is " + p.hotel[i].getName() + ", Stars:" + p.hotel[i].getStars() + ", Duration Days: " + p.reservation[i].getStayDurationDays() + ", Date:" + p.reservation[i].getCheckinDate() + "\n");
                        }
                     }
